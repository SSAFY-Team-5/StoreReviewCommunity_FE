<template>
  <div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
      <button
        @click="goBack"
        class="inline-flex items-center space-x-2 text-slate-500 hover:text-slate-950 font-semibold text-xs transition-colors"
      >
      ...
      <div class="flex items-center space-x-2">
        ...
        <button
          ...
          class="px-3 py-1.5 bg-rose-50 border border-rose-100 text-rose-600 hover:bg-rose-100 hover:border-rose-200 text-xs font-semibold rounded-xl transition-all flex items-center space-x-1.5"
        >
          <AppIcon name="trash" />
          <span>삭제</span>
        </button>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8 space-y-6">
          <div class="space-y-3">
            <div class="flex items-center space-x-2">
              <span
                v-if="post.storeName"
                class="bg-indigo-50 text-indigo-600 text-xs font-semibold px-2.5 py-1 rounded-full flex items-center"
              >
      ...
      <div class="space-y-6">
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-4">
          ...
          <div
            v-else
            class="flex flex-col items-center justify-center py-10 px-4 text-center text-slate-400 bg-slate-50 rounded-xl border border-dashed border-slate-200"
          >
